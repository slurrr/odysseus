...
def _cached_model_scan_script(model_dirs: list[str] | None = None) -> str:
    ...
    lines = [
        ...
        "    # If p itself is a model folder (or a folder of direct GGUF symlinks), include it.",
        "    root_is_model = False",
        "    for root, dirs, fns in safe_walk(p):",
        "        for fn in fns:",
        "            if fn.lower().endswith('.gguf') or fn == 'config.json' or fn.endswith('.safetensors') or fn.endswith('.bin'):",
        "                root_is_model = True; break",
        "        break",
        "    if root_is_model:",
        "        rid = os.path.basename(os.path.normpath(p)) or p",
        "        if rid not in seen:",
        "            gguf_files = collect_ggufs(p)",
        "            seen.add(rid)",
        "            sz, nf = 0, 0",
        "            for dp, _, fns in safe_walk(p):",
        "                for fn in fns:",
        "                    try: nf += 1; sz += os.path.getsize(os.path.join(dp, fn))",
        "                    except Exception: pass",
        "            is_diff = os.path.exists(os.path.join(p, 'model_index.json'))",
        "            models.append({'repo_id':rid,'size_bytes':sz,'nb_files':nf,'has_incomplete':False,'path':os.path.dirname(os.path.normpath(p)) or p,'is_local_dir':True,'is_diffusion':is_diff,'is_gguf':bool(gguf_files),'gguf_files':gguf_files})",
        "    for d in sorted(os.listdir(p)):",
        "        if d.startswith('.'): continue",
        "        if d.startswith('models--'): continue",
        "        fp = os.path.join(p, d)",
        "        if not os.path.isdir(fp) or not safe_path(fp): continue",
